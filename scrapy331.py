...
class patentSpyer:
    def __init__(self,postData='手机'):
        ...
        'searchCondition.searchExp':postData,
    ...
    def readHtml(self,opener,post,url,header):
        ...
        if (html=="timeout"):
            print "网络不好"
            raise TypeError('Readhtml函数请求未达 !')
        """
#        if html==403:
 #           ipport=getIP()
 ...
 #           html = urllib2.urlopen('http://XXXX').read()
    ...
    def getEncoParamSendreq(self,currentpage,rawtext_decode):
        ...
        for j in range(0,10):# range 10 each page has ten items
            ...
            f=open("C:/Users/shixiong/Desktop/download/patent"+patclass+"class"+str(y)+"page_"+str(x)+"item.txt",'w')
            f.write(contents)
