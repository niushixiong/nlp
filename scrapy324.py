...
def get(opener,req,retries=10):
    try:
        ...
        if isinstance(e.reason, socket.timeout):
            print "request timeout and rerequest"
            return get(opener,req,retries-1)
        if hasattr(e,"reason"):
            print "URLError,we fail to send to server"
            print "reason:",e.reason
            if retries>0:
                return get(opener,req,retries-1)
            else:
                print '10 times request Failed',req
                return ''
        elif hasattr(e,"code"):
            print "the server cannot fulfill the req"
    ...
    def getEncodeParams(self,pages=1):
        ...
        for i in range(1,pages):
            ...
            if currentpage==1:
                ...
                for j in range(0,10):# this 1 need to change to items 10
                    ...
                    #访问该链接#
                    #get the param
                    # 原来是 urllib2.urlopen(req)
                    #result = urllib2.urlopen(req,timeout=) 
                    paramsDeal=get(opener, req,10) 
                    ...
                    #原来是html=urllib2.urlopen(req,timeout=)
